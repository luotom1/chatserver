...
Redis::~Redis()
{
    if (_publish_context != NULL)
    {
        redisFree(_publish_context);
    }

    if (_subcribe_context != NULL)
    {
...
{
    //SUBSCRIBE命令本身会造成线程阻塞，直到有消息过来，这里只做订阅通道，不接收消息
    //通道消息的接收专门在observer_channel_message函数中的独立线程中进行
    //只负责发送命令，不阻塞接收redis server响应消息，否则和observer_channel_message线程抢占资源
